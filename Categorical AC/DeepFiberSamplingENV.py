...
class PolytopeENV(Env):
    def __init__(self, 
        ...
        if self.reward_cost == None:
            print("Reward dictionary is None")
            return
...
#         state = self.initial_states[state_indx]
        if self.visited_states.shape[0] > 1:
            state_indx = random.randint(0,self.visited_states.shape[0]-1)
        state = self.visited_states[state_indx,:]
        self.state = state
    ...
    def step(self, action):
        ...
        if np.all(self.action == 0):
            ...
            if all(coord >= 0 for coord in next_state):
                ...
                reward_direction = abs(self.reward_cost(self.reward_list, self.prev_state)-self.reward_cost(self.reward_list, next_state))
#                 print("Reward dIrections: ", reward_direction)
