...
def create_fiber_sampling_erdos_renyi_graph(file, initial_states, total_nodes, p, graph_num, weight_min, weight_max):
    
    for g in range(graph_num):
        
        G_full = create_fully_connected_graph(total_nodes, True, 0, weight_min, weight_max)
        G = nx.erdos_renyi_graph(total_nodes, p, seed=None, directed=False)

#         subgraph_edges = G_rand.edges  # Edges from G
#         G = G_full.edge_subgraph(subgraph_edges).copy()
        
        edge_weights = extract_weights_and_edges(G_full)
        print(edge_weights)
        ...
        reward_list = compute_reward_map(objective_table, sorted(list(G_full.nodes())))
    ...
    return available_actions, initial_states, reward_list
